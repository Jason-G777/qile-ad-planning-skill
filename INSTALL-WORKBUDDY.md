...
进一步验证“实际调用”是否生效：输入“请说明这份 Brief 触发了哪些路线、每条路线产出了什么判断，再给方案”。回答应区分读取与调用，并先完成策略核，再进入创意；不能只罗列已安装模块。
...
- 需要去AI味时可直接说“调用 Humanizer，把这段改得像人写，但不要改变策略与信息量”；该模块无需额外安装运行组件。
